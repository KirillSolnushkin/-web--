...
<body>
  ...
  <form action="" method="post">
    ...
    <input type="number" id="art_Id" name="art_Id" required><br>
  ...
  function getAllComments($link)
  {
    ...
    if (!$result) {
      outputText("Помилка: " . mysqli_error($link));
    } else {
      echo "<h2>Comments</h2>";
      echo "<table border='1'>";
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["notes"]) && $_POST["notes"] == "notes") {
    ...
    if (!$link) {
      outputText("Помилка з'єднання: " . mysqli_connect_error());
    } else {
      $created = date('Y-m-d H:i:s');
      $query = "INSERT INTO Notes (Title, Article, Created) VALUES ('$name', '$article', '$created')";

      if (mysqli_query($link, $query)) {
        outputText("Новий запис успішно додано до таблиці Notes.");
        header("Location: {$_SERVER['REQUEST_URI']}");
        exit();
      } else {
        outputText("Помилка при виконанні запиту:: " . mysqli_error($link));
      }
    }
  }
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["comment"]) && $_POST["comment"] == "comment") {
    ...
    if (!$link) {
      ...
      if (mysqli_query($link, $query)) {
        outputText("Новий запис успішно додано до таблиці Comments.");
        header("Location: {$_SERVER['REQUEST_URI']}");
        exit();
      } else {
        outputText("Помилка при виконанні запиту:: " . mysqli_error($link));
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["delete_comment"]) && $_POST["delete_comment"] == "delete") {
    ...
    if (!$link) {
      ...
      if (mysqli_query($link, $query)) {
        outputText("Коментар успішно видалено.");
        header("Location: {$_SERVER['REQUEST_URI']}");
        exit();
      } else {
        outputText("Помилка при виконанні запиту: " . mysqli_error($link));
